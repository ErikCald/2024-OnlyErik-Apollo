...
public class AutoRoutines extends SubsystemBase {
    ...
    public void registerCommandsToPathplanner() {
        ...
        NamedCommands.registerCommand(
                "simpleShooter", CombinedCommands.simpleShootNoteSpeaker(0.4));

        NamedCommands.registerCommand(
                "MakeShooterSpin",
                new Shooter_PID_Tuner(() -> Config.ShooterConstants.subwooferRPM));
        ...
        //     Commands.runOnce(()->
        // IntakeSubsystem.getInstance().setMode(IntakeStatesVoltage.Modes.STOP))));

        // NamedCommands.registerCommand("turnOnIntake", (
        //         Commands.runOnce(()->
        // IntakeSubsystem.getInstance().setMode(IntakeStatesVoltage.Modes.INTAKE))));

        NamedCommands.registerCommand("simpleIntake", (new MakeIntakeMotorSpin(7.0, 0)));
        ...
        NamedCommands.registerCommand(
                ...
                new SelectByAllianceCommand(
                        new PhotonMoveToTarget(
                                PhotonPositions.MIDDLE_SPEAKER_BLUE.destination, false, false),
                        new PhotonMoveToTarget(
                                PhotonPositions.MIDDLE_SPEAKER_RED.destination, false, false)));
        ...
        NamedCommands.registerCommand(
                ...
                new SetArm(() -> 90)
                        .until(
                                () ->
                                        ArmSubsystem.getInstance().getPosition()
                                                > Math.toRadians(82)));
        NamedCommands.registerCommand(
                "ArmPickup", new SetArm(() -> Config.ArmSetPoints.INTAKE.angleDeg));
        NamedCommands.registerCommand(
                "ArmKitbotShot",
                new SetArm(() -> Config.ArmSetPoints.SPEAKER_KICKBOT_SHOT.angleDeg));

        // Working but behaves weirdly with pathplanner
        NamedCommands.registerCommand(
                "VisionScoreSourceSideClose",
                autoSpeakerScore(
                        8,
                        2,
                        3750,
                        ...
                        SwerveSubsystem.getInstance()
                                .setOdometryCommand(
                                        new Pose2d(
                                                0,
                                                0,
                                                SwerveSubsystem.rotateForAlliance(
    ...
    public static Command autoSpeakerScore(
        ...
        // log the centered note in the intake rollers
        Debouncer notRotatingDebouncer = new Debouncer(0.3);
        Command intakeShooterSequence =
                Commands.sequence(
                        ...
                        Commands.parallel(
                                ...
                                new WaitCommand(0.1)
                                        .andThen(new Shooter_PID_Tuner(() -> shooterSpeed))));
        ...
        Command driveToPositionAndPrepare =
                Commands.deadline(
                        ...
                        new SelectByAllianceCommand(
                                new PhotonMoveToTarget(
                                        bluePosition.destination,
                                        bluePosition.direction,
                                        false,
                                        true),
                                new PhotonMoveToTarget(
                                        redPosition.destination,
        ...
        Command scoreNote =
                Commands.parallel(
                                ...
                                new Shooter_PID_Tuner(
                                        () -> shooterSpeed), // Continue to hold shooter voltage
                                new SetArm(
                                        () -> armAngleDeg), // Continue to hold arm in the correct
                                // position
        ...
        return Commands.sequence(
                CombinedCommands.forcefulTimeoutCommand(
                        preparingTimeoutSeconds, driveToPositionAndPrepare),
                scoreNote);
    }
}
