...
public class TeleopSwerve extends Command {
    ...
    public static void resetAccelerationLimiters() {
        ...
        translationLimiter.reset(
                SwerveSubsystem.getInstance().getFieldRelativeSpeeds().vxMetersPerSecond);
        strafeLimiter.reset(
                SwerveSubsystem.getInstance().getFieldRelativeSpeeds().vyMetersPerSecond);
        rotationLimiter.reset(
                SwerveSubsystem.getInstance().getFieldRelativeSpeeds().omegaRadiansPerSecond);
    }
    ...
    // protected double calculateRotationVal() {
    //   rotationVal = MathUtil.applyDeadband(-driver.getRawAxis(rotationAxis),
    // Config.Swerve.stickDeadband)
    //     * speed.angularSpeed;
    ...
    public void execute() {
        ...
        s_Swerve.drive(
                new ChassisSpeeds(
                        calculateTranslationVal(), calculateStrafeVal(), calculateRotationVal()),
                isFieldRelative,
                true);

        // if (rotationVal != 0.0 ||
        // Math.toDegrees(s_Swerve.getRobotRelativeSpeeds().omegaRadiansPerSecond) < 10) {
        //   prevHeading = s_Swerve.getHeading();
        // }
    }
}
