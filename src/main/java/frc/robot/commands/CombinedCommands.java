...
public class CombinedCommands {
    ...
    public static Command visionScoreTeleopSimple(
        ...
        Command idleBling =
                new ProxyCommand(
                        Commands.idle(BlingSubsystem.getINSTANCE())
                                ...
                                .withName("ProxiedIdleBling"));
        Command turnOffBling =
                new ProxyCommand(new BlingCommand(BlingColour.DISABLED).withName("TurnOffBling"));

        // Wait for vision data to be available
        Command waitForVisionData =
                Commands.deadline(
                        new ProxyCommand(
                                new SelectByAllianceCommand(
                                                PhotonSubsystem.getInstance()
                                                        .getWaitForDataCommand(bluePosition.id),
                                                PhotonSubsystem.getInstance()
                                                        .getWaitForDataCommand(redPosition.id))
                                        .withName("ProxiedWaitForVisionData")),
                        new ProxyCommand(
                                Commands.parallel(
                                                new IntakeControl(
                                                        false), // Reverse note until not touching
                                                // shooter
                                                new WaitCommand(0.2)
                                                        .andThen(
                                                                new Shooter_PID_Tuner(
                                                                        () -> shooterSpeed)))
                                        .withName("ProxiedReverseNoteAndSpinupShooter")));

        // Wait for all subsytems to get ready
        Command waitForAllSubsytems =
                Commands.parallel(
                        new WaitUntilCommand(
                                () ->
                                        ShooterSubsystem.getInstance().getVelocityRPM()
                        ...
                        new WaitUntilCommand(
                                () ->
                                        SwerveSubsystem.getInstance()
                                                        .isAtPose(
                                                                PhotonConfig.POS_TOLERANCE,
                                                                PhotonConfig.ANGLE_TOLERANCE)
                                                && !SwerveSubsystem.getInstance()
                                                        .isChassisMoving(
                                                                PhotonConfig.VEL_TOLERANCE)));

        // Control all subsystems commands
        Command controlAllSubsystems =
                Commands.parallel(
                        new WaitUntilCommand(keepArmLoweredUntil)
                                .andThen(new SetArm(() -> armAngleDeg)),
                        // new
                        // ProxyCommand(centerNoteThenSpinUpShooer(shooterSpeed).withName("ProxiedCenterNoteThenSpinUpShooter")),
                        new ProxyCommand(
                                Commands.parallel(
                                                new IntakeControl(
                                                        false), // Reverse note until not touching
                        ...
                        new SelectByAllianceCommand(
                                PhotonSubsystem.getInstance()
                                        .getAprilTagCommand(bluePosition, driverJoystick, true),
                                PhotonSubsystem.getInstance()
                                        .getAprilTagCommand(redPosition, driverJoystick, true)),
        ...
        // Rumble command
        Command rumble =
                new RumbleJoystick(driverJoystick, RumbleType.kBothRumble, 0.7, 0.3, false);
                        ...
                        Commands.deadline(
                                Commands.sequence(
                                        forcefulTimeoutCommand(
                                                preparingTimeoutSeconds, waitForAllSubsytems),
                                        scoreNote),
                ...
                .finallyDo(
                        () -> {
                            ...
                            Commands.sequence(
                                            ...
                                            new ScheduleCommand(turnOffBling))
                                    .withName("DelayTurnOffBling")
                                    .schedule();
    ...
    public static Command visionScoreTeleopStateful(
        ...
        Command idleSwerve =
                Commands.idle(SwerveSubsystem.getInstance()).withName("IdlingSwerveStateful");

        // Prepare the robot to score
        Command driveToPositionAndPrepare =
                Commands.parallel(
                        new SetArm(() -> armAngleDeg),
                        new WaitCommand(0.1)
                                .andThen(ShooterSubsystem.getInstance().speedUpForSpeakerCommand()),
                        Commands.sequence(
                                new SelectByAllianceCommand(
                                        PhotonSubsystem.getInstance()
                                                .getAprilTagCommand(
                                                        bluePosition, driverJoystick, false),
                                        PhotonSubsystem.getInstance()
                                                .getAprilTagCommand(
                                                        redPosition, driverJoystick, false)),
                                new ScheduleCommand(
                                        idleSwerve) // Hog the swerve subsystem to prevent the
                                // teleop command from running
                                ));
        ...
        Command scoreNote =
                Commands.parallel(
                                ...
                                new SetArm(() -> armAngleDeg) // Continue to hold arm in the correct
                                // position
                                )
                        .withTimeout(scoringTimeoutSeconds);
        ...
        // Sequence preparing then scoring
        return Commands.sequence(
                        forcefulTimeoutCommand(preparingTimeoutSeconds, driveToPositionAndPrepare),
                        scoreNote,
                        Commands.runOnce(
                                () ->
                                        ShooterSubsystem.getInstance()
                                                .setMode(ShooterModes.STOP_SHOOTER)))
                .finallyDo(
                        () -> {
                            rumble.schedule(); // Rumble the joystick to notify the driver
     ...
     * @param redPosition PhotonPosition for the red alliance
     */
    public static Command centerSpeakerVisionShot(
            CommandXboxController driver,
            PhotonPositions bluePosition,
            PhotonPositions redPosition) {
        BooleanSupplier keepArmLoweredUntil =
                () -> {
                    return PhotonSubsystem.getInstance().getTargetPos().getY()
                                    - SwerveSubsystem.getInstance().getPose().getY()
                            < 0.5;
                };
     ...
     * @param redPosition PhotonPosition for the red alliance
     */
    public static Command podiumSourceSideSpeakerVisionShot(
            CommandXboxController driver,
            PhotonPositions bluePosition,
            PhotonPositions redPosition) {
        // BooleanSupplier keepArmLoweredUntil = () -> {
        //   return PhotonSubsystem.getInstance().getTargetPos().getY() -
        // SwerveSubsystem.getInstance().getPose().getY() > 0.5;
        // };
     ...
     * @param redPosition PhotonPosition for the red alliance
     */
    public static Command statefulSpeakerScoreWithVision(
            CommandXboxController driver,
            ArmSetPoints armAngle,
            PhotonPositions bluePosition,
            PhotonPositions redPosition) {
        return CombinedCommands.visionScoreTeleopStateful(
                driver, 25, 4, armAngle.angleDeg, bluePosition, redPosition);
    }
