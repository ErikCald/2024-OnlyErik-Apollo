...
public class PhotonMoveToTarget extends Command {
    ...
    public PhotonMoveToTarget(
            Translation2d _targetOffset,
            Rotation2d _desiredHeading,
            boolean _isWaypoint,
            boolean neverEnd) {
        addRequirements(SwerveSubsystem.getInstance());
        addRequirements(PhotonSubsystem.getInstance());
    ...
    public void execute() {
        ...
        if (centerTarget) {
            SwerveSubsystem.getInstance()
                    .driveToPose(new Pose2d(setPoint.plus(targetOffset), rotationSetPoint));
        } else {
            SwerveSubsystem.getInstance()
                    .driveToPose(new Pose2d(setPoint.plus(targetOffset), desiredHeading));
        }
    }
    ...
    public boolean isFinished() {
        ...
        if (isWaypoint) {
            return SwerveSubsystem.getInstance()
                    .isAtPose(
                            PhotonConfig.WAYPOINT_POS_TOLERANCE,
                            PhotonConfig.WAYPOINT_ANGLE_TOLERANCE);
        } else {
            return SwerveSubsystem.getInstance()
                            .isAtPose(PhotonConfig.POS_TOLERANCE, PhotonConfig.ANGLE_TOLERANCE)
