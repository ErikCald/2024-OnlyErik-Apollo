...
public final class Config {
     ...
     *
     * <p>0. Connect to the robot to the robot using a usb cable or the wifi network. 1. Using a
     * tool like Git Bash or putty, ssh into admin@roboRIO-2706-FRC.local (ssh
     * admin@roboRIO-2706-FRC.local) a. There is no password on a freshly flashed roboRIO 2. Go up a
     * directory (cd ..) 3. cd into lvuser/ (cd lvuser/) 4. Create a new file called robot.conf
     * (touch robot.conf) 5. Open the file with vi (vi robot.conf) 6. Press i to enter insert mode
     * 7. Add an integer denoting the robot id. If it's the first robot, use 0, second use 1 etc. 8.
     * Press [ESC] followed by typing :wq in order to save and quit 9. To verify this worked type:
     * more robot.conf 10. If it displays the value you entered, it was successful 11. Type exit to
     ...
     * ROBOT IDs
     *
     * <p>ID 0: Competition Robot (Crescendo) (NEEDS UPDATE ON robot.conf) ID 1: Simulation of Comp
     * Robot (Crescendo in Simulation) ID 2: Poseidon (Charged Up) (NEEDS UPDATE ON robot.conf) ID
     * 3: Clutch (Rapid React) (NEEDS UPDATE ON robot.conf)
     */
