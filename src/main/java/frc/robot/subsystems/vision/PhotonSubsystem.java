...
public class PhotonSubsystem extends SubsystemBase {
    ...
    private PhotonSubsystem() {
        ...
        pubSetPoint =
                photonTable
                        .getDoubleArrayTopic("PhotonAprilPoint")
        ...
        pub3DTagsDebugMsg =
                photonTable.getStringTopic("3DTagsDebugMsg").publish(PubSubOption.periodic(0.02));
        subOverrideTagID = photonTable.getIntegerTopic("OVERIDEID").getEntry(-1);
        subOverrideTagID.setDefault(-1);
        SmartDashboard.putData(
                "command reset id", Commands.runOnce(() -> reset((int) subOverrideTagID.get())));
        reset(-1);
        ...
        try {
            ...
            photonPoseEstimator =
                    new PhotonPoseEstimator(
                            ...
                            camera1,
                            PhotonConfig.cameraTransform);
        } catch (Exception e) {
            aprilTagFieldLayout = null;
            PhotonConfig.USE_3D_TAGS = false;
            DriverStation.reportError(
                    "Merge's PhotonSubsystem failed to create the apriltag layout. Using 2D"
                            + " apriltags instead of 3D.",
                    false);
        }

        // Intake camera snapping photons
        NetworkTable intakeTable =
                NetworkTableInstance.getDefault().getTable(PhotonConfig.frontCameraName);
    ...
    public Command getWaitForDataCommand(int tagid) {
        return new FunctionalCommand(
                () -> reset(tagid), () -> {}, (interrupted) -> {}, () -> hasData(), this);
    }
    ...
    public Command getAprilTagCommand(
            PhotonPositions spacePositions, CommandXboxController driverStick, boolean neverEnd) {
        Command moveToTargetCommands;
        if (spacePositions.hasWaypoint) {
            moveToTargetCommands =
                    Commands.sequence(
                            new PhotonMoveToTarget(
                                    spacePositions.waypoint, spacePositions.direction, true, false),
                            new PhotonMoveToTarget(
                                    spacePositions.destination,
                                    spacePositions.direction,
                                    false,
                                    neverEnd));
        } else {
            moveToTargetCommands =
                    new PhotonMoveToTarget(
                            spacePositions.destination, spacePositions.direction, false, neverEnd);
        }

        return Commands.sequence(
                // new ProxyCommand(getWaitForDataCommand(spacePositions.id)), // Proxy this command
                // to prevent PhotonSubsystem requirement conflicting with PhotonMoveToTarget's
                // requirements
                new ScheduleCommand(
                        new RumbleJoystick(driverStick, RumbleType.kBothRumble, 0.7, 1, true)),
                new ProxyCommand(
                        moveToTargetCommands.withName(
                                "ProxiedSwerveAprilTagCommand")) // Proxy this command to prevent
                // SwerveSubsystem requirement
                // conflicting with
                // WaitForDataCommand's
                // requirements
                );
    }
    ...
    private double range(double y) {
        ...
        return (Config.PhotonConfig.APRIL_HEIGHTS[id_array] - PhotonConfig.CAMERA_HEIGHT)
                / Math.tan(y);
    }
    ...
    private Pose2d convertToField(double range, Rotation2d yaw, Pose2d odometryPose) {
        ...
        Translation2d robotToTargetRELATIVE =
                robotRotated.plus(PhotonConfig.cameraOffset.getTranslation());
    ...
    public void periodic() {
        ...
        if (PhotonConfig.USE_3D_TAGS) {
            ...
            if (!((tagsInFrame.contains(3) && tagsInFrame.contains(4))
                    || (tagsInFrame.contains(7) && tagsInFrame.contains(8)))) {
                pub3DTagsDebugMsg.accept(
                        "3&4 or 7&8 not in frame. Tags in frame: " + tagsInFrame.toString());
                return;
            }
            ...
            // Grab the pose from when the image was taken to compensate for how much the robot has
            // moved since the image was taken
            Optional<Pose2d> odometryPose =
                    SwerveSubsystem.getInstance()
                            .getPoseAtTimestamp(optEstPose.get().timestampSeconds);
            if (odometryPose.isEmpty()) {
                pub3DTagsDebugMsg.accept(
                        "No odometry pose at timestamp: " + optEstPose.get().timestampSeconds);
                return;
            }

            // Create a transform that maps the change in Pose between the robot estimated position
            // and the true tag position
            Transform2d robotToTarget =
                    new Transform2d(
                            optEstPose.get().estimatedPose.toPose2d(), tagPose.get().toPose2d());

            // Map the position of the tag relative to the current odometry pose with latency
            // compensation
            fieldToTarget = odometryPose.get().plus(robotToTarget);

            pub3DTagsDebugMsg.accept(
                    "Got pose. Transform2d from robot to tag: " + robotToTarget.toString());
            ...
            if (result.hasTargets()) {
                ...
                Optional<Pose2d> optPose =
                        SwerveSubsystem.getInstance()
                                .getPoseAtTimestamp(result.getTimestampSeconds());
        ...
        if (fieldToTarget != null) {
            ...
            targetRotation =
                    Rotation2d.fromDegrees(
                            filteryaw.calculate(fieldToTarget.getRotation().getDegrees()));
            numSamples++;

            // publish to networktables
            pubSetPoint.accept(
                    new double[] {targetPos.getX(), targetPos.getY(), targetRotation.getRadians()});
        }
    }
