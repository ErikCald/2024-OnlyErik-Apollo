 ...
 */
public class LocalADStarLoggable implements Pathfinder {
    ...
    public final Set<GridPosition> staticObstacles = new HashSet<>();
    ...
    private List<PathPoint> createPathPoints(
        ...
        for (int i = 1; i < path.size() - 1; i++) {
            if (!walkable(
                    simplifiedPath.get(simplifiedPath.size() - 1), path.get(i + 1), obstacles)) {
        ...
        for (int i = 1; i < fieldPosPath.size() - 1; i++) {
            ...
            Translation2d prevControl1 =
                    last.minus(anchor1).times(SMOOTHING_CONTROL_PCT).plus(anchor1);
            Translation2d nextControl1 =
                    new Translation2d(controlDist, anchor1.minus(prevControl1).getAngle())
                            .plus(anchor1);

            Translation2d prevControl2 =
                    new Translation2d(controlDist, anchor2.minus(next).getAngle()).plus(anchor2);
            Translation2d nextControl2 =
                    next.minus(anchor2).times(SMOOTHING_CONTROL_PCT).plus(anchor2);
    ...
    public GridPosition getGridPos(Translation2d pos) {
    ...
    public Translation2d gridPosToTranslation2d(GridPosition pos) {
