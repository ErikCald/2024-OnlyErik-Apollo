...
public class SwerveSubsystem extends SubsystemBase {
    ...
    private SwerveSubsystem() {
        ...
        swerveOdometry =
                new SwerveDriveOdometry(
                        Config.Swerve.swerveKinematics, getYaw(), getPositions(), new Pose2d());
        ...
        pidControlX =
                new ProfiledPIDController(1, 0.0, 0.2, new TrapezoidProfile.Constraints(1, 1));
        pidControlY =
                new ProfiledPIDController(1, 0.0, 0.2, new TrapezoidProfile.Constraints(1, 1));
        pidControlRotation =
                new ProfiledPIDController(
                        4.0, 0, 0.4, new TrapezoidProfile.Constraints(4 * Math.PI, 8 * Math.PI));
    }
    ...
    public Command setHeadingCommand(Rotation2d rotation2d) {
        return Commands.runOnce(
                () -> resetOdometry(new Pose2d(getPose().getTranslation(), rotation2d)));
    }
    ...
    public boolean isAtPose(double tol, double angleTol) {
        return Math.abs(currentX - desiredX) < tol
    ...
    public void periodic() {
        ...
        if (isChassisMoving() == false
                && ++moduleSynchronizationCounter > 5
                && isSwerveNotSynched()) {
            synchSwerve();
    ...
    public void driveRobotRelative(ChassisSpeeds robotRelativeSpeeds) {
        ...
        SwerveModuleState[] targetStates =
                Config.Swerve.swerveKinematics.toSwerveModuleStates(targetSpeeds);
