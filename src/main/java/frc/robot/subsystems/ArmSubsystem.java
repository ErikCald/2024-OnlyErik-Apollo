...
public class ArmSubsystem extends SubsystemBase {
    private static ArmSubsystem instance =
            null; // static object that contains all movement controls
    ...
    private ArmSubsystem() {
        ...
        configureSpark(
                ...
                () ->
                        (m_arm.setSoftLimit(
                                ...
                                (float) (Config.ArmConfig.arm_forward_limit))));
        configureSpark(
                "Arm sets soft limits reverse",
                () ->
                        (m_arm.setSoftLimit(
                                SoftLimitDirection.kReverse,
                                (float) (Config.ArmConfig.arm_reverse_limit))));
        configureSpark(
                "Arm enables soft limits forward",
                () ->
                        (m_arm.enableSoftLimit(
                                SoftLimitDirection.kForward, Config.ArmConfig.SOFT_LIMIT_ENABLE)));
        configureSpark(
                "Arm enable soft limit reverse",
                () ->
                        (m_arm.enableSoftLimit(
                                SoftLimitDirection.kReverse, Config.ArmConfig.SOFT_LIMIT_ENABLE)));
        ...
        configureSpark(
                ...
                () ->
                        m_absEncoder.setPositionConversionFactor(
                                Config.ArmConfig.armPositionConversionFactor));
        configureSpark(
                "Absolute Encoder set velocity conversion factor",
                () ->
                        m_absEncoder.setVelocityConversionFactor(
                                Config.ArmConfig.armVelocityConversionFactor));
        configureSpark(
                "Absolute encoder set zero offset",
                () ->
                        m_absEncoder.setZeroOffset(
                                Math.toRadians(Config.ArmConfig.armAbsEncoderOffset)));
        ...
        m_armMomentToVoltage =
                ArmTuningTable.getDoubleTopic("MomentToVoltage")
        ...
        m_armPosPub =
                ArmDataTable.getDoubleTopic("MeasuredAngleDeg")
                        .publish(PubSubOption.periodic(0.02));
        m_armVelPub =
                ArmDataTable.getDoubleTopic("MeasuredVelocity")
                        .publish(PubSubOption.periodic(0.02));
        m_armFFTestingVolts =
                ArmDataTable.getDoubleTopic("FFTestingVolts").publish(PubSubOption.periodic(0.02));
        m_targetAngle =
                ArmDataTable.getDoubleTopic("TargetAngleDeg").publish(PubSubOption.periodic(0.02));
    ...
    public void updatePID0Settings() {
        ...
        configureSpark(
                "Arm set Output Range",
                () ->
                        (m_pidControllerArm.setOutputRange(
                                Config.ArmConfig.min_output, Config.ArmConfig.max_output)));
    }
    ...
    public void updatePID1Settings() {
        ...
        configureSpark(
                "Arm set far Iz", () -> m_pidControllerArm.setIZone(ArmConfig.arm_far_iZone, 1));
    }
    ...
    public void setJointAngle(double angle) {
        double clampedAngle =
                MathUtil.clamp(
                        angle,
                        Math.toRadians(Config.ArmConfig.MIN_ARM_ANGLE_DEG),
        ...
        // m_pidControllerArm.setReference((targetPos), ControlType.kPosition, 0,
        // calculateFF(clampedAngle));
        m_pidControllerArm.setReference(
                targetPos + Math.toRadians(ArmConfig.shiftEncoderRange),
    ...
    private double calculateFF(double encoder1Rad) {
        // double ArmMoment = Config.ArmConfig.ARM_FORCE *
        // (Config.ArmConfig.LENGTH_ARM_TO_COG*Math.cos(encoder1Rad));
